...
- d9c6dfe: Fix #33: library consumers can now emit `.d.ts` when re-exporting inferred
  schema types. The field classes (`ObjectField`, `StringField`, `NumberField`,
  …) are now exported (as values, so they can also be used with `instanceof`),
  making `typeof schema` nameable, and the internal `_parse` method (with
  `ParseContext` / `InternalParseResult`) is hidden from the public type surface
  via `stripInternal`. This unblocks the schema-once pattern
