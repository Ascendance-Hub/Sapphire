...
/**
 * FK walking in drizzle-orm ^0.45:
 *   import { getTableConfig } from 'drizzle-orm/pg-core'
...
describe('pg refs', () => {
  it('basic ref → integer column with references()', () => {
    ...
    expect(postsTable.author.columnType).toBe('PgInteger')
  ...
  it('lazy resolution — build Post before User, resolve later', () => {
    ...
    const tables = new DrizzleTableRegistry()
    const postSchema = a.object({ title: a.string(), author: a.ref('User') }).name('Post')
    // Build Post first — callback NOT invoked yet, so this must not throw.
    const postsTable = toDrizzleSchema(postSchema.toSchema(), {
      dialect: 'pg',
      tableName: 'posts',
      tables,
    ...
    // Invoke the callback → must resolve cleanly.
    const cfg = getTableConfig(postsTable)
    const ref = cfg.foreignKeys[0]!.reference()
    expect(ref.foreignColumns[0]!.name).toBe('id')
  })
  ...
  it('registry keyed by node.name — ref resolves even when SQL tableName differs', () => {
    const a = new Sapphire()
    const userSchema = a.object({ name: a.string() }).name('User')
    const postSchema = a.object({ title: a.string(), author: a.ref('User') }).name('Post')
    const tables = new DrizzleTableRegistry()
    // SQL table name differs from schema name.
    toDrizzleSchema(userSchema.toSchema(), {
      dialect: 'pg',
      tableName: 'app_users',
      tables,
    ...
    // Registry must contain 'User' (node.name), not 'app_users'.
    expect(tables.has('User')).toBe(true)
    expect(tables.has('app_users')).toBe(false)

    const cfg = getTableConfig(postsTable)
    const ref = cfg.foreignKeys[0]!.reference()
    expect(ref.foreignColumns[0]!.name).toBe('id')
    // Drizzle reports the foreign table's SQL name.
    const fkTableCfg = getTableConfig(ref.foreignTable as any)
    expect(fkTableCfg.name).toBe('app_users')
  })
})
