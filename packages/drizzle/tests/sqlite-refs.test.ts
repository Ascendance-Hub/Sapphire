...
/**
 * FK walking in drizzle-orm ^0.45.2 sqlite-core:
 *   import { getTableConfig } from 'drizzle-orm/sqlite-core'
...
describe('sqlite refs', () => {
  it('basic ref → integer column with references()', () => {
    ...
    toDrizzleSchema(userSchema.toSchema(), {
      dialect: 'sqlite',
      tableName: 'sqlite_users',
      tables,
    })
    const postsTable = toDrizzleSchema(postSchema.toSchema(), {
      dialect: 'sqlite',
      tableName: 'sqlite_posts',
      tables,
    }) as any

    expect(postsTable.author.columnType).toBe('SQLiteInteger')
  ...
  it('cycle (User ↔ Post) — both tables build, both refs resolve', () => {
    ...
    const usersTable = toDrizzleSchema(userSchema.toSchema(), {
      dialect: 'sqlite',
      tableName: 'sqlite_users_cy',
      tables,
    }) as any
    const postsTable = toDrizzleSchema(postSchema.toSchema(), {
      dialect: 'sqlite',
      tableName: 'sqlite_posts_cy',
      tables,
    }) as any
  ...
  it('missing target → build succeeds, callback throws with name + "not registered"', () => {
    ...
    const postsTable = toDrizzleSchema(postSchema.toSchema(), {
      dialect: 'sqlite',
      tableName: 'sqlite_posts_missing',
      tables,
    }) as any
    const cfg = getTableConfig(postsTable)
